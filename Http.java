...
public class Http extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		servletOut.println(
				...
				+ "<form method='post' action= '"+ response.encodeRedirectURL("Http")+"'>"
				+ "<label>Enter your guess</label>"
				+ "<input name='num' type=\"text\"></input>"
				+ "<br/><input type=\"submit\" name=\"submit\" value='Check number'>"
				+ "<p>Session will be invalid after 10 seconds</p>"
				+"</form>"
				+"</body>"
				+ "</html>"				
				);
	...
	protected void printcongrats(PrintWriter servletOut){
		servletOut.println("<html><body><h1>Congratulation, you are correct!</h1></body></html>");
	}
	protected void printlarger(PrintWriter servletOut, int guess){
		servletOut.println("<html><body><h3>You are incorrect, "+
